...
TimeVector=Dt:Dt:5; % Start, increment, end
...
% Sum of each column
% We take the transpose to ensure that we are summing across trials
...
eg1 = sin(0:0.1:pi)*10 - 5;
temp=makeSparsity(eg1);
fprintf(['Sinusoid sparsity for eg1= ' num2str(temp) '\n'])

eg2 = sin(0:0.1:pi)*10 + 5;
temp=makeSparsity(eg2);
fprintf(['Sinusoid sparsity for eg2= ' num2str(temp) '\n'])

eg3 = sin(0:0.1:pi)*5 - 5;
temp=makeSparsity(eg3);
fprintf(['Sinusoid sparsity for eg3= ' num2str(temp) '\n'])

eg4 = sin(0:0.1:pi)*5 + 5;
temp=makeSparsity(eg4);
fprintf(['Sinusoid sparsity for eg4= ' num2str(temp) '\n'])

eg5 = sin(0:0.1:pi)*10 + 10;
temp=makeSparsity(eg5);
fprintf(['Sinusoid sparsity for eg5= ' num2str(temp) '\n'])

eg6 = sin(0:0.1:pi)*10 + 0;
temp=makeSparsity(eg6);
fprintf(['Sinusoid sparsity for eg6= ' num2str(temp) '\n'])

%{
Sparsity is directly proportional to the scaling factor
With increase in offset, there is a decrease in sparsity, inversely
proportional. 
When the scaling factor and the offset are changed by the same ratio, the
sparsity doesn't change
%}
...
% sin is positive over the first two quadrants
Lambda=FR*Dt; %Lambda, which is a threshold
...
TimePoints = t/Dt;
N = 100;
equi_spaced_angles = linspace(0,pi,TimePoints/t);
% Diving 0 to pi into 1000 equal points
FR = ones(1,5000)*2; % Since baseline firing is 2Hz
FR(2001:3000) = 2 + sin(equi_spaced_angles)*8;
% Only for 2 to 3 sec it's sinusoidal
Lambda=FR*Dt;
Lambda_AllTrials=repmat(Lambda,100,1);
...
figure;
plot(TimeVector, FR);
title("Plotting with baseline 2Hz and peak 10Hz")
xlabel('Time in seconds')
ylabel('FR')
...
q21 = makeSparsity(NewTuningCurve);
fprintf(['Tuned spike matrix sparsity= ' num2str(q21) '\n'])
